...
extension LoginItemStatus {
    ...
    /// Может ли **только что записанная** регистрация вступить в силу сама, без нового
    /// действия пользователя.
    ///
    /// Отвечает ровно на один вопрос: уместна ли после успешной записи plist фраза «вступит
    /// при следующем входе». Чистая функция статуса — её зовёт строка автозапуска, и её
    /// проверяет тест без `ServiceManagement`.
    ///
    /// - `enabled` — вступать нечему, регистрация уже действует;
    /// - `disabledByUser` — **не вступит**. Пользователь выключил пункт в System Settings, и
    ///   система помнит этот выбор поверх файла: перезапись plist статус не меняет,
    ///   `systemStatus` остаётся `2` (измерено ручным чеклистом TASK-008, подпункт 5).
    ///   Обещать здесь следующий вход значит успокаивать ложью ровно в тот момент, когда
    ///   пользователь только что действовал и склонен поверить. Включить обратно можно там
    ///   же, где выключено, — и это единственный правдивый ответ (DEC-006);
    /// - `notRegistered` и `unknown` — вступить может: `unknown(3)` наблюдался переходным
    ///   состоянием сразу после записи и становился `enabled` без входа в систему, за время
    ///   из вилки (0, 29 мин].
